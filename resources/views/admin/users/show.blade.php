@extends('admin.layouts.layout')

@section('title', 'Detail User')

@section('content')
<div class="space-y-6">
    <!-- Header -->
    <div class="flex items-center justify-between">
        <div>
            <a href="{{ route('admin.users.index') }}" 
                class="text-sm text-tumbloo-gray hover:text-tumbloo-black inline-flex items-center mb-2">
                <svg class="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                </svg>
                Kembali ke Daftar User
            </a>
            <h1 class="text-3xl font-bold text-tumbloo-black">{{ $user->name }}</h1>
        </div>
        <div class="flex items-center space-x-3">
            @if($user->role == 'admin')
            <span class="badge badge-danger">Admin</span>
            @elseif($user->role == 'seller')
            <span class="badge badge-info">Seller</span>
            @else
            <span class="badge badge-success">Customer</span>
            @endif
            
            <a href="{{ route('admin.users.edit', $user->id) }}" class="btn-primary btn-sm">
                <svg class="h-4 w-4 mr-1 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                </svg>
                Edit User
            </a>
        </div>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <!-- Main Content -->
        <div class="lg:col-span-2 space-y-6">
            <!-- User Information -->
            <div class="card p-6">
                <h2 class="text-xl font-bold text-tumbloo-black mb-4">Informasi User</h2>
                
                <div class="flex items-start space-x-4 mb-6">
                    <div class="h-20 w-20 rounded-full bg-tumbloo-black flex items-center justify-center text-tumbloo-white font-bold text-2xl">
                        {{ substr($user->name, 0, 1) }}
                    </div>
                    
                    <div class="flex-1">
                        <h3 class="text-2xl font-bold text-tumbloo-black">{{ $user->name }}</h3>
                        <p class="text-tumbloo-gray mt-1">{{ $user->email }}</p>
                        <p class="text-sm text-tumbloo-gray mt-1">Member sejak {{ $user->created_at->format('d M Y') }}</p>
                    </div>
                </div>

                <div class="divider"></div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="label">Role</label>
                        <p class="text-tumbloo-gray">{{ ucfirst($user->role) }}</p>
                    </div>

                    <div>
                        <label class="label">Email Verified</label>
                        <p class="text-tumbloo-gray">
                            @if($user->email_verified_at)
                            <span class="text-green-600 font-medium">Terverifikasi</span>
                            @else
                            <span class="text-red-600 font-medium">Belum Terverifikasi</span>
                            @endif
                        </p>
                    </div>

                    <div>
                        <label class="label">Terdaftar Sejak</label>
                        <p class="text-tumbloo-gray">{{ $user->created_at->format('d M Y H:i') }}</p>
                    </div>

                    <div>
                        <label class="label">Terakhir Diupdate</label>
                        <p class="text-tumbloo-gray">{{ $user->updated_at->format('d M Y H:i') }}</p>
                    </div>
                </div>
            </div>

            <!-- Store Information (if seller) -->
            @if($user->store)
            <div class="card p-6">
                <h2 class="text-xl font-bold text-tumbloo-black mb-4">Informasi Toko</h2>
                
                <div class="flex items-start space-x-4">
                    @if($user->store->logo)
                    <img src="{{ Storage::url($user->store->logo) }}" alt="{{ $user->store->name }}" 
                        class="h-16 w-16 rounded-lg object-cover border-2 border-tumbloo-gray-light">
                    @else
                    <div class="h-16 w-16 rounded-lg bg-tumbloo-black flex items-center justify-center text-tumbloo-white font-bold text-xl">
                        {{ substr($user->store->name, 0, 1) }}
                    </div>
                    @endif
                    
                    <div class="flex-1">
                        <div class="flex items-center space-x-2">
                            <h3 class="text-lg font-bold text-tumbloo-black">{{ $user->store->name }}</h3>
                            @if($user->store->is_verified)
                            <span class="badge badge-success">Verified</span>
                            @else
                            <span class="badge badge-warning">Pending</span>
                            @endif
                        </div>
                        <p class="text-tumbloo-gray mt-1">{{ Str::limit($user->store->description, 100) }}</p>
                        <a href="{{ route('admin.stores.show', $user->store->id) }}" 
                            class="text-sm text-tumbloo-black hover:text-tumbloo-dark font-medium mt-2 inline-block">
                            Lihat Detail Toko →
                        </a>
                    </div>
                </div>
            </div>
            @endif

            <!-- Buyer Information -->
            @if($user->buyer)
            <div class="card p-6">
                <h2 class="text-xl font-bold text-tumbloo-black mb-4">Informasi Pembeli</h2>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @if($user->buyer->address)
                    <div>
                        <label class="label">Alamat</label>
                        <p class="text-tumbloo-gray">{{ $user->buyer->address }}</p>
                    </div>
                    @endif

                    @if($user->buyer->phone)
                    <div>
                        <label class="label">Nomor Telepon</label>
                        <p class="text-tumbloo-gray">{{ $user->buyer->phone }}</p>
                    </div>
                    @endif

                    @if($user->buyer->city)
                    <div>
                        <label class="label">Kota</label>
                        <p class="text-tumbloo-gray">{{ $user->buyer->city }}</p>
                    </div>
                    @endif

                    @if($user->buyer->postal_code)
                    <div>
                        <label class="label">Kode Pos</label>
                        <p class="text-tumbloo-gray">{{ $user->buyer->postal_code }}</p>
                    </div>
                    @endif
                </div>
            </div>
            @endif

            <!-- Transaction History -->
            <div class="card p-6">
                <h2 class="text-xl font-bold text-tumbloo-black mb-4">Riwayat Transaksi</h2>
                
                @if($user->transactions && $user->transactions->count() > 0)
                <div class="overflow-x-auto">
                    <table class="table">
                        <thead>
                            <tr>
                                <th>Invoice</th>
                                <th>Toko</th>
                                <th>Total</th>
                                <th>Status</th>
                                <th>Tanggal</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($user->transactions->take(10) as $transaction)
                            <tr>
                                <td class="font-medium">{{ $transaction->invoice }}</td>
                                <td>{{ $transaction->store->name }}</td>
                                <td class="font-semibold">Rp {{ number_format($transaction->grand_total, 0, ',', '.') }}</td>
                                <td>
                                    @if($transaction->payment_status == 'delivered')
                                    <span class="badge badge-success">Selesai</span>
                                    @elseif($transaction->payment_status == 'shipped')
                                    <span class="badge badge-info">Dikirim</span>
                                    @elseif($transaction->payment_status == 'processing')
                                    <span class="badge badge-warning">Diproses</span>
                                    @else
                                    <span class="badge badge-danger">{{ ucfirst($transaction->payment_status) }}</span>
                                    @endif
                                </td>
                                <td class="text-sm">{{ $transaction->created_at->format('d M Y') }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
                @if($user->transactions->count() > 10)
                <p class="text-sm text-tumbloo-gray mt-4 text-center">
                    Dan {{ $user->transactions->count() - 10 }} transaksi lainnya
                </p>
                @endif
                @else
                <p class="text-tumbloo-gray text-center py-8">Belum ada transaksi</p>
                @endif
            </div>
        </div>

        <!-- Sidebar -->
        <div class="space-y-6">
            <!-- Quick Stats -->
            <div class="card p-6">
                <h2 class="text-xl font-bold text-tumbloo-black mb-4">Statistik</h2>
                
                <div class="space-y-4">
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-tumbloo-gray">Status</span>
                        @if($user->role == 'admin')
                        <span class="badge badge-danger">Admin</span>
                        @elseif($user->role == 'seller')
                        <span class="badge badge-info">Seller</span>
                        @else
                        <span class="badge badge-success">Customer</span>
                        @endif
                    </div>
                    
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-tumbloo-gray">Total Transaksi</span>
                        <span class="text-sm font-medium text-tumbloo-black">{{ $user->transactions ? $user->transactions->count() : 0 }}</span>
                    </div>

                    @if($user->store)
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-tumbloo-gray">Produk Toko</span>
                        <span class="text-sm font-medium text-tumbloo-black">{{ $user->store->products->count() }}</span>
                    </div>
                    @endif

                    <div class="flex justify-between items-center">
                        <span class="text-sm text-tumbloo-gray">Member Sejak</span>
                        <span class="text-sm font-medium text-tumbloo-black">{{ $user->created_at->diffForHumans() }}</span>
                    </div>
                </div>
            </div>

            <!-- Actions -->
            <div class="card p-6">
                <h2 class="text-xl font-bold text-tumbloo-black mb-4">Aksi</h2>
                
                <div class="space-y-3">
                    <a href="{{ route('admin.users.edit', $user->id) }}" class="w-full btn-primary btn-sm">
                        <svg class="h-4 w-4 mr-2 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                        </svg>
                        Edit User
                    </a>

                    @if($user->role != 'admin')
                    <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="w-full btn-secondary btn-sm bg-red-600 hover:bg-red-700 text-white" 
                            onclick="return confirm('Apakah Anda yakin ingin menghapus user ini? Data tidak dapat dikembalikan!')">
                            <svg class="h-4 w-4 mr-2 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                            </svg>
                            Hapus User
                        </button>
                    </form>
                    @endif
                </div>
            </div>
        </div>
    </div>
</div>
@endsection