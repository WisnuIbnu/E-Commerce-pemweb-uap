<header>
    <div class="header-container">
        ...
        <ul class="nav-center">
            ...
            <li><a href="{{ route('collection') }}" class="{{ request()->routeIs('collection') ? 'active' : '' }}">Collection</a></li>
            @auth
                <li>
                    <a href="{{ route('cart.show') }}" class="{{ request()->routeIs('cart.show') ? 'active' : '' }}" style="display: flex; align-items: center; gap: 6px;">
                        ...
                        @if(session('cart') && count(session('cart'))  > 0)
                            <span style="background: var(--primary); color: black; padding: 2px 6px; border-radius: 10px; font-size: 0.75rem; font-weight: 600;">{{ count(session('cart')) }}</span>
                        @endif
                    </a>
                </li>
                @if(Auth::user()->isSeller())
                    <li><a href="{{ route('seller.dashboard') }}">Seller Hub</a></li>
                    <li><a href="{{ route('seller.products') }}">Products</a></li>
                    <li><a href="{{ route('seller.orders') }}">Orders</a></li>
                @endif
                <li><a href="{{ route('transaction.history') }}">My Orders</a></li>
            @endauth
        </ul>
